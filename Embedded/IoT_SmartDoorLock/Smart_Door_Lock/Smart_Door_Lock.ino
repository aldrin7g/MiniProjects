...
const char* ssid = "Virus#404";  //WiFi Name
const char* password = "password2";  //WiFi Password
...
String BOTtoken = "7954159070:AAGs24er2Bg1ckezhzKNWm3HH-NxADe_wek";  // your Bot Token (Get from Botfather)
String chatId = "-4614042640";
...
String unlockDoor(){  
  if (Firebase.getInt(fbdo, "/Lock")){ 
      int val = fbdo.intData();
      if(!val) return "Door Already Unlocked. /lock";
      else{
        Firebase.setInt(fbdo, "/LockBot", 0);
        sendPhoto = true;
        delay(100);
...
String lockDoor(){
  if (Firebase.getInt(fbdo, "/Lock")){ 
      int val = fbdo.intData();
      if(val) return "Door Already Locked. /unlock";
      else{
        Firebase.setInt(fbdo, "/LockBot", 1);
        delay(100);
...
String sendPhotoTelegram(){
  ...
  if (clientTCP.connect(myDomain, 443)) {
   ...
   Serial.println("Connected to " + String(myDomain));
    ...
    String boundary = "----BOUNDARY----"; // Unique boundary string
...
void loop(){
  ...
  if (Firebase.getInt(fbdo, "/Photo")){
      int val = fbdo.intData();
      if (val == 1)
        sendPhoto = true;
        val = 0;
        Firebase.setInt(fbdo, "/Photo", 0);
  }
